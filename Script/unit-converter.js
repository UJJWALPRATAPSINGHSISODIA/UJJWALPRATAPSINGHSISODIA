...
function CalculateUnit(sourceForm, targetForm) {
  ...
  var sourceValue = sourceForm.unit_converter_unit_input.value;

  // First check if the user has given numbers or anything that can be made to one...
  sourceValue = parseFloat(sourceValue);
  if (!isNaN(sourceValue) || sourceValue == 0) {
    // If we can make a valid floating-point number, put it in the text box and convert!
    sourceForm.unit_converter_unit_input.value = sourceValue;
...
function ConvertFromTo(sourceForm, targetForm) {
  ...
  propIndex = document.unit_converter_property_form.unit_converter_the_menu.selectedIndex;

  // Let's determine what unit are we converting FROM (i.e. source) and the factor needed to convert that unit to the base unit.
  sourceIndex = sourceForm.unit_converter_unit_menu.selectedIndex;
  sourceFactor = factor[propIndex][sourceIndex];

  // Cool! Let's do the same thing for the target unit - the units we are converting TO:
  targetIndex = targetForm.unit_converter_unit_menu.selectedIndex;
  ...
  result = sourceForm.unit_converter_unit_input.value;
  ...
  targetForm.unit_converter_unit_input.value = result;
}

// This fragment initializes the property dropdown menu using the data defined above in the 'Data Definitions' section
window.onload = function(e) {
  FillMenuWithArray(document.unit_converter_property_form.unit_converter_the_menu, property);
  UpdateUnitMenu(document.unit_converter_property_form.unit_converter_the_menu, document.unit_converter_form_A.unit_converter_unit_menu);
  UpdateUnitMenu(document.unit_converter_property_form.unit_converter_the_menu, document.unit_converter_form_B.unit_converter_unit_menu)
}

// Restricting textboxes to accept numbers + navigational keys only
document.getElementByClass('unit-converter-numbersonly').addEventListener('keydown', function(e) {
